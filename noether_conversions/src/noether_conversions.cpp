...
{
static geometry_msgs::msg::Pose pose3DtoPoseMsg(const std::tuple<float, float, float, float, float, float>& p)
{
  ...
  geometry_msgs::msg::Pose pose_msg = tf2::toMsg(eigen_pose);
  return std::move(pose_msg);
}

bool convertToPCLMesh(const shape_msgs::msg::Mesh& mesh_msg, pcl::PolygonMesh& mesh)
{
...
bool convertToMeshMsg(const pcl::PolygonMesh& mesh, shape_msgs::msg::Mesh& mesh_msg)
{
  ...
  for (std::size_t i = 0; i < mesh.polygons.size(); i++)
  {
    ...
    mesh_msg.triangles[i].vertex_indices[0] = vertices.vertices[0];
    mesh_msg.triangles[i].vertex_indices[1] = vertices.vertices[1];
    mesh_msg.triangles[i].vertex_indices[2] = vertices.vertices[2];
  }
...
bool loadPLYFile(const std::string& filename, shape_msgs::msg::Mesh& mesh_msg)
{
  pcl::PolygonMesh mesh;
  return (pcl::io::loadPLYFile(filename, mesh) >= 0) && convertToMeshMsg(mesh, mesh_msg);
}

visualization_msgs::msg::Marker createMeshMarker(const std::string& mesh_file,
                                            const std::string& ns,
                           ...
                           bool silent)
{
  ...
  for (std::size_t i = 0; i < mesh.polygons.size(); i++)
  {
    const auto& vert = mesh.polygons[i].vertices;
...
visualization_msgs::msg::MarkerArray convertToAxisMarkers(const noether_msgs::msg::ToolPaths& toolpaths,
                                                     const std::string& frame_id,
                                                     const std::string& ns,
                                                     ...
                                                     const std::tuple<float, float, float, float, float, float>& offset)
{
  ...
  {
    for (const geometry_msgs::msg::PoseArray& segment : tool_path.segments)
    {
      for (const geometry_msgs::msg::Pose& pose : segment.poses)
      {
...
convertToArrowMarkers(const noether_msgs::msg::ToolPaths& toolpaths,
                      const std::string& frame_id,
                      const std::string& ns,
                      const std::size_t start_id,
                      ...
                      const std::tuple<float, float, float, float, float, float>& offset)
{
  ...
  auto transformPoint = [](const geometry_msgs::msg::Pose& pose_msg,
                           const geometry_msgs::msg::Point& p_msg) -> geometry_msgs::msg::Point {
    auto new_p_msg = p_msg;
    Eigen::Isometry3d transform;
    Eigen::Vector3d p, new_p;
    tf2::fromMsg(pose_msg, transform);
    tf2::fromMsg(p_msg, p);
    return tf2::toMsg(transform * p);
  };
  ...
  for (const noether_msgs::msg::ToolPath& tool_path : toolpaths.paths)
  {
    for (const geometry_msgs::msg::PoseArray& segment : tool_path.segments)
    {
      ...
      for (std::size_t i = 1; i < segment.poses.size(); i++)
      {
        ...
        geometry_msgs::msg::Point p_start = transformPoint(pose_msg, segment.poses[i - 1].position);
        geometry_msgs::msg::Point p_end = transformPoint(pose_msg, segment.poses[i].position);
...
visualization_msgs::msg::MarkerArray
convertToDottedLineMarker(const noether_msgs::msg::ToolPaths& toolpaths,
                          const std::string& frame_id,
                          const std::string& ns,
                          const std::size_t& start_id,
                          const std::tuple<float, float, float, float, float, float>& offset,
