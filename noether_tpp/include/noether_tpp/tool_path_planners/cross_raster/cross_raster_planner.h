...
{
/**
 * @ingroup cross_direction_generators
 * @brief Interface for generating the direction of cross raster paths.
 * @details This interface only defines the cross raster path direction.
 ...
 */
struct CrossDirectionGenerator
{
  using Ptr = std::unique_ptr<CrossDirectionGenerator>;
  using ConstPtr = std::unique_ptr<const CrossDirectionGenerator>;

  virtual ~CrossDirectionGenerator() = default;
 ...
 */
struct CrossOriginGenerator
{
  using Ptr = std::unique_ptr<CrossOriginGenerator>;
  using ConstPtr = std::unique_ptr<const CrossOriginGenerator>;

  virtual ~CrossOriginGenerator() = default;
  virtual Eigen::Vector3d generate(const pcl::PolygonMesh& mesh) const = 0;
};
 ...
 */
class CrossRasterPlanner : public ToolPathPlanner
{
public:
  CrossRasterPlanner(CrossDirectionGenerator::ConstPtr dir_gen, CrossOriginGenerator::ConstPtr origin_gen);
...
protected:
  ...
  CrossDirectionGenerator::ConstPtr dir_gen_;
  CrossOriginGenerator::ConstPtr origin_gen_;
 ...
 */
struct CrossRasterPlannerFactory : public ToolPathPlannerFactory
{
  /** @brief Distance between waypoints on the same raster line (m) */
  ...
  std::function<std::unique_ptr<const CrossDirectionGenerator>()> direction_gen;
  /** @brief Function for creating a raster origin generator */
  std::function<std::unique_ptr<const CrossOriginGenerator>()> origin_gen;
};
