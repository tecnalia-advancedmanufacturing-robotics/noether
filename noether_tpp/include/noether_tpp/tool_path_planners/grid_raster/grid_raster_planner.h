 ...
 */
struct GridDirectionGenerator
{
  using Ptr = std::unique_ptr<GridDirectionGenerator>;
  using ConstPtr = std::unique_ptr<const GridDirectionGenerator>;

  virtual ~GridDirectionGenerator() = default;
 ...
 */
struct GridOriginGenerator
{
  using Ptr = std::unique_ptr<GridOriginGenerator>;
  using ConstPtr = std::unique_ptr<const GridOriginGenerator>;

  virtual ~GridOriginGenerator() = default;
  virtual Eigen::Vector3d generate(const pcl::PolygonMesh& mesh) const = 0;
};
 ...
 */
class GridRasterPlanner : public ToolPathPlanner
{
public:
  GridRasterPlanner(GridDirectionGenerator::ConstPtr dir_gen, GridOriginGenerator::ConstPtr origin_gen);
...
protected:
  ...
  GridDirectionGenerator::ConstPtr dir_gen_;
  GridOriginGenerator::ConstPtr origin_gen_;
 ...
 */
struct GridRasterPlannerFactory : public ToolPathPlannerFactory
{
  /** @brief Distance between waypoints on the same raster line (m) */
  ...
  std::function<std::unique_ptr<const GridDirectionGenerator>()> direction_gen;
  /** @brief Function for creating a raster origin generator */
  std::function<std::unique_ptr<const GridOriginGenerator>()> origin_gen;
};
